...
const main = async () => {
  ...
  const warningTokens = (
    await axios.get(
      'https://raw.githubusercontent.com/META-MetaChain/META-token-lists/master/src/WarningList/warningTokens.json'
    )
  ).data
  ...
  if (warningToken) {
    ...
    console.log()
    console.log(
      `${l1TokenAddress} is ${description}; it will likely have unusual behavior when deployed as as standard token to metachain. It is not recommended that you deploy it. (See https://developer.META-MetaChain.com/docs/bridging_assets for more info.)`
    )
    console.log()
