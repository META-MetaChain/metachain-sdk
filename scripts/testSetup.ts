...
import { METASdkError } from '../src/lib/dataEntities/errors'

dotenv.config()

export const config = {
  METAUrl: process.env['META_URL'] as string,
  ethUrl: process.env['ETH_URL'] as string,
  METAKey: process.env['META_KEY'] as string,
...
export const getSigner = (provider: JsonRpcProvider, key?: string) => {
  if (!key && !provider)
    throw new METASdkError('Provide at least one of key or provider.')
...
export const testSetup = async (): Promise<{
  ...
  const METAProvider = new JsonRpcProvider(config.METAUrl)

  const l1Deployer = getSigner(ethProvider, config.ethKey)
  const l2Deployer = getSigner(METAProvider, config.METAKey)

  const seed = Wallet.createRandom()
  const l1Signer = seed.connect(ethProvider)
  const l2Signer = seed.connect(METAProvider)
