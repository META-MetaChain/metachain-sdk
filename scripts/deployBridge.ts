...
import { StandardMETAERC20__factory } from '../src/lib/abi/factories/StandardMETAERC20__factory'
...
import { METAMulticall2__factory } from '../src/lib/abi/factories/METAMulticall2__factory'
...
export const deployErc20L2 = async (deployer: Signer) => {
  ...
  const standardMETAERC20 = await new StandardMETAERC20__factory()
    .connect(deployer)
    .deploy()
  await standardMETAERC20.deployed()

  const beacon = await new UpgradeableBeacon__factory()
    .connect(deployer)
    .deploy(standardMETAERC20.address)
  ...
  const multicall = await new METAMulticall2__factory()
    .connect(deployer)
    .deploy()
  await multicall.deployed()
  console.log('multicall', multicall.address)
