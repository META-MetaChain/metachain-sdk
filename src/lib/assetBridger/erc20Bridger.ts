...
import { L2metachainGateway__factory } from '../abi/factories/L2metachainGateway__factory'
...
import { IMETAToken__factory } from '../abi/factories/IMETAToken__factory'

import { WithdrawalInitiatedEvent } from '../abi/L2metachainGateway'
...
import { METASdkError, MissingProviderMETASdkError } from '../dataEntities/errors'
...
> {
  ...
  public async getL1ERC20Address(
    ...
    const METAERC20 = L2GatewayToken__factory.connect(erc20L2Address, l2Provider)
    const l1Address = await METAERC20.functions.l1Address().then(([res]) => res)
  ...
  public async withdraw(
    ...
    if (!SignerProviderUtils.signerHasProvider(params.l2Signer)) {
      throw new MissingProviderMETASdkError('l2Signer')
    }
...
export class AdminErc20Bridger extends Erc20Bridger {
  /**
   * Register a custom token on the metachain bridge
   * See https://developer.META-MetaChain.com/docs/bridging_assets#the-metachain-generic-custom-gateway for more details
   * @param l1TokenAddress Address of the already deployed l1 token. Must inherit from https://developer.META-MetaChain.com/docs/sol_contract_docs/md_docs/META-bridge-peripherals/tokenbridge/ethereum/icustomtoken.
   * @param l2TokenAddress Address of the already deployed l2 token. Must inherit from https://developer.META-MetaChain.com/docs/sol_contract_docs/md_docs/META-bridge-peripherals/tokenbridge/metachain/iMETAtoken.
   * @param l1Signer The signer with the rights to call registerTokenOnL2 on the l1 token
   * @param l2Provider metachain rpc provider
   * @returns
   */
  public async registerCustomToken(
    ...
    if (!SignerProviderUtils.signerHasProvider(l1Signer)) {
      throw new MissingProviderMETASdkError('l1Signer')
    }
    ...
    const l2Token = IMETAToken__factory.connect(l2TokenAddress, l2Provider)
