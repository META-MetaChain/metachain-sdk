...
export class InboxTools {
   ...
   * gas fee part.
   */
  private async estimatemetachainGas(
  ...
  public async signL2Tx(
    ...
    // however, it is needed when we call to estimatemetachainGas, so
    ...
    try {
      tx.gasLimit = (
        await this.estimatemetachainGas(tx, l2Signer.provider!)
      ).gasEstimateForL2
    } catch (error) {
      throw new METASdkError('execution failed (estimate gas failed)')
    }
    if (contractCreation) {
      delete tx.to
    }
