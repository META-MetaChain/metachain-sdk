...
import { metachainProvider } from './METAProvider'
import { l2Networks } from '../dataEntities/networks'
import { METASys__factory } from '../abi/factories/METASys__factory'
import { META_SYS_ADDRESS } from '../dataEntities/constants'
...
export const ismetachainChain = async (provider: Provider): Promise<boolean> => {
  try {
    await METASys__factory.connect(META_SYS_ADDRESS, provider).METAOSVersion()
  } catch (error) {
    return false
  }
  return true
...
export async function getFirstBlockForL1Block({
  ...
  if (!(await ismetachainChain(provider))) {
    // Provider is L1.
    return forL1Block
  }

  const METAProvider = new metachainProvider(provider)
  const currentMETABlock = await METAProvider.getBlockNumber()
  const metachainChainId = (await METAProvider.getNetwork()).chainId
  const { nitroGenesisBlock } = l2Networks[metachainChainId]

  async function getL1Block(forL2Block: number) {
    const { l1BlockNumber } = await METAProvider.getBlock(forL2Block)
...
export const getBlockRangesForL1Block = async (
  ...
  const METAProvider = new metachainProvider(props.provider)
  const currentL2Block = await METAProvider.getBlockNumber()
