...
metachain SDK includes methods for [serializing parameters](https://developer.META-MetaChain.com/docs/special_features#parameter-byte-serialization) for a solidity method into a single byte array to minimize calldata. It uses the following schema:
...
import { METAAddressTable__factory } from '../abi/factories/METAAddressTable__factory'
import { METAAddressTable } from '../abi/METAAddressTable'

import { META_ADDRESS_TABLE_ADDRESS } from '../dataEntities/constants'
...
export const getAddressIndex = (() => {
  ...
  let METAAddressTable: METAAddressTable | undefined
  ...
  return async (address: string, signerOrProvider: Signer | Provider) => {
    ...
    METAAddressTable =
      METAAddressTable ||
      METAAddressTable__factory.connect(
        META_ADDRESS_TABLE_ADDRESS,
        signerOrProvider
      )
    const isRegistered = await METAAddressTable.addressExists(address)
    if (isRegistered) {
      const index = (await METAAddressTable.lookup(address)).toNumber()
...
export const argSerializerConstructor = (
  ...
  return async (params: PrimativeOrPrimativeArray[]) => {
    return await serializeParams(params, async (address: string) => {
      return await getAddressIndex(address, METAProvider)
    })
  }
}
...
const formatPrimative = (value: PrimativeType) => {
  if (isAddress(value)) {
    ...
    throw new METASdkError('unsupported type')
  }
}
/**
