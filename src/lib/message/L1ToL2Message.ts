...
import { METARetryableTx__factory } from '../abi/factories/METARetryableTx__factory'
import { META_RETRYABLE_TX_ADDRESS } from '../dataEntities/constants'
import {
  SignerProviderUtils,
  SignerOrProvider,
...
export abstract class L1ToL2Message {
  ...
  public static calculateSubmitRetryableId(
    ...
    const fields: any[] = [
      ...
      // when destAddress is 0x0, METAos treat that as nil
    ...
    // metachain submit retry transactions have type 0x69
...
export class L1ToL2MessageReader extends L1ToL2Message {
    ...
    while (fromBlock.number < maxBlock) {
      ...
      const redeemEvents = await eventFetcher.getEvents(
        ...
        contract => contract.filters.RedeemScheduled(this.retryableCreationId),
        {
          ...
          address: META_RETRYABLE_TX_ADDRESS,
        }
      )
  ...
  public static async getLifetime(l2Provider: Provider): Promise<BigNumber> {
    const METARetryableTx = METARetryableTx__factory.connect(
      META_RETRYABLE_TX_ADDRESS,
      l2Provider
    )
    return await METARetryableTx.getLifetime()
  }
  ...
  public async getTimeout(): Promise<BigNumber> {
    ...
    return await METARetryableTx.getTimeout(this.retryableCreationId)
  }
  ...
  public getBeneficiary(): Promise<string> {
    ...
    return METARetryableTx.getBeneficiary(this.retryableCreationId)
  }
}
...
export class L1ToL2MessageWriter extends L1ToL2MessageReader {
  public constructor(
    ...
    if (!l2Signer.provider)
      throw new METASdkError('Signer not connected to provider.')
  }
  ...
  public async redeem(overrides?: Overrides): Promise<RedeemTransaction> {
    ...
    if (status === L1ToL2MessageStatus.FUNDS_DEPOSITED_ON_L2) {
      ...
      const redeemTx = await METARetryableTx.redeem(this.retryableCreationId, {
        ...overrides,
      })
  ...
  public async cancel(overrides?: Overrides): Promise<ContractTransaction> {
    ...
    if (status === L1ToL2MessageStatus.FUNDS_DEPOSITED_ON_L2) {
      ...
      return await METARetryableTx.cancel(this.retryableCreationId, overrides)
    } else {
      throw new METASdkError(
  ...
  public async keepAlive(overrides?: Overrides): Promise<ContractTransaction> {
    ...
    if (status === L1ToL2MessageStatus.FUNDS_DEPOSITED_ON_L2) {
      ...
      return await METARetryableTx.keepalive(this.retryableCreationId, overrides)
    } else {
      throw new METASdkError(
...
export class EthDepositMessage {
  ...
  public static calculateDepositTxId(
    ...
    // https://github.com/META-MetaChain/go-ethereum/blob/07e017aa73e32be92aadb52fa327c552e1b7b118/core/types/META_types.go#L302-L308
    const fields = [
      formatNumber(chainId),
      zeroPad(formatNumber(msgNum), 32),
    ...
    // metachain eth deposit transactions have type 0x64
    const rlpEnc = ethers.utils.hexConcat([
      '0x64',
      ethers.utils.RLP.encode(fields),
    ])
  ...
  private static parseEthDepositData(eventData: string): {
    ...
    // https://github.com/META-MetaChain/nitro/blob/aa84e899cbc902bf6da753b1d66668a1def2c106/contracts/src/bridge/Inbox.sol#L242
