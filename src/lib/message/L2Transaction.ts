...
import { RedeemScheduledEvent } from '../abi/METARetryableTx'
...
/**
 * Extension of ethers-js TransactionReceipt, adding metachain-specific functionality
 */
export class L2TransactionReceipt implements TransactionReceipt {
  ...
  public getL2ToL1Events(): L2ToL1TransactionEvent[] {
    ...
    const nitroLogs = parseTypedLogs(METASys__factory, this.logs, 'L2ToL1Tx')
  ...
  public getRedeemScheduledEvents(): EventArgs<RedeemScheduledEvent>[] {
    return parseTypedLogs(METARetryableTx__factory, this.logs, 'RedeemScheduled')
  }
  ...
  public async getL2ToL1Messages<T extends SignerOrProvider>(
    ...
    if (!provider) throw new METASdkError('Signer not connected to provider.')
  ...
  public async getBatchNumber(l2Provider: providers.JsonRpcProvider) {
    ...
    const METAProvider = new metachainProvider(l2Provider)
    const rec = await METAProvider.getTransactionReceipt(this.transactionHash)
    if (rec == null)
