...
import { L2ToL1TxEvent } from '../abi/METASys'
...
import { getBlockRangesForL1Block, ismetachainChain, wait } from '../utils/lib'
import { getL2Network } from '../dataEntities/networks'
import { NodeCreatedEvent, RollupUserLogic } from '../abi/RollupUserLogic'
import { metachainProvider } from '../utils/METAProvider'
import { METABlock } from '../dataEntities/rpc'
  ...
  public static async getL2ToL1Events(
    ...
    return (
      await eventFetcher.getEvents(
        ...
        { ...filter, address: META_SYS_ADDRESS }
      )
...
export class L2ToL1MessageReaderNitro extends L2ToL1MessageNitro {
  ...
  public async getOutboxProof(l2Provider: Provider) {
    ...
    if (!sendRootSize)
      throw new METASdkError('Node not yet created, cannot get proof.')
  ...
  private async getBlockFromNodeLog(
    ...
    const metachainProvider = new metachainProvider(l2Provider)

    if (!log) {
      console.warn('No NodeCreated events found, defaulting to block 0')
      return metachainProvider.getBlock(0)
    }

    const parsedLog = this.parseNodeCreatedAssertion(log)
    const l2Block = await metachainProvider.getBlock(
  ...
  private async getBlockFromNodeNum(
    ...
    // If L1 is metachain, then L2 is an Orbit chain.
    if (await ismetachainChain(this.l1Provider)) {
      try {
  ...
  public async getFirstExecutableBlock(
    ...
    if (status !== L2ToL1MessageStatus.UNCONFIRMED)
      throw new METASdkError('L2ToL1Msg expected to be unconfirmed')
